...
import { ISWAP_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof ISWAP_PROGRAM_ADDRESS,
  TAccountIswap extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountIswap extends string
        ? WritableAccount<TAccountIswap>
        : TAccountIswap,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountIswap extends string = string> = {
  iswap: Address<TAccountIswap>;
};

export function getIncrementInstruction<
  TAccountIswap extends string,
  TProgramAddress extends Address = typeof ISWAP_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountIswap>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountIswap> {
  // Program address.
  const programAddress = config?.programAddress ?? ISWAP_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    iswap: { value: input.iswap ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.iswap)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountIswap>;
